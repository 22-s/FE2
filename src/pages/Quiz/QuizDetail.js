...
const QuizDetail = ({ route }) => {
  ...
  const fetchQuizzes = async (quizId) => {
    ...
    try {
      const response = await axios.get(
        `https://22s.store/api/quiz/${currentQuizId}`,
        { headers }
      );
  ...
  const updateQuizId = (newQuizId) => {
    console.log("업데이트 퀴즈 아이디: "+ newQuizId);
    fetchQuizzes(newQuizId+1);
  };

  const nextQuiz = (currentQuizId) => {
    if(isSubmit) {
      if(quizId == lastQuizId) Alert("마지막 퀴즈입니다.");
      console.log("다음 퀴즈 : " + currentQuizId);
      fetchQuizzes(currentQuizId+1);
    } else {
      Alert("현재 퀴즈를 풀어야 넘어갈 수 있습니다.");
    }
  }

  const prevQuiz = (currentQuizId) => {
    if(isSubmit) {
      if(currentQuizId == firstQuizId) Alert("이전 퀴즈가 없습니다.");
      console.log("다음 퀴즈 : " + currentQuizId);
      fetchQuizzes(currentQuizId-1);
    } else {
      Alert("현재 퀴즈를 풀어야 넘어갈 수 있습니다.");
  ...
  const addReview = async () => {
    ...
    try {
      if (bookmark) {
        ...
        // 복습하기 추가 API 호출
        await axios.post(`https://22s.store/api/quiz/${currentQuizId}/review`, {
          headers,
        });
